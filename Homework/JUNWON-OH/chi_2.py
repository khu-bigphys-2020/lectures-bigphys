...
for n in tqdm(range(1, 101)):
    for i in range(1000):
        ...
        if loss < bestLoss:
            ...
            bestA = A
            bestLoss = loss
    chiSquare_i = [((y[j] - f(x[j], bestA, best_k)) ** 2) / (f(x[j], bestA, best_k) ** 2)
                   for j in range(100)]
    normChiSquareList.append(sum(chiSquare_i) / 100)
    kHistory.append(best_k)
    AHistory.append(bestA)
    lossHistory.append(bestLoss)
...
plt.show()
# %%
plt.hist(normChiSquareList, bins=60)
plt.show()
# %%
